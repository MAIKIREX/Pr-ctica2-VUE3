<template>
  <div>
    <ModalForm v-model:modelValue="showModalNuevo">
      <RegisterPacienteView @on-register="onRegister" />
    </ModalForm>
    <ModalForm v-model:modelValue="showModalEdit">
      <EditPacienteView @on-update="onUpdate" :item="itemToEdit" />
    </ModalForm>
...
export default {
  ...
  methods: {
    getList() {
      axios
        .get(`http://localhost:4000/pacientes?q=${this.textToSearch}`)
        .then((response) => {
    ...
    eliminar(id) {
      if (confirm("¿Está seguro de eliminar el registro?")) {
        axios
          .delete(`http://localhost:4000/pacientes/${id}`)
          .then(() => {
            this.getList();
